...
let index=Math.max(0,queue.findIndex(a=>a.id===initialId)),player=null,frame=null,muted=true,ytReady=false,generation=0;
...
function show(){generation++;try{player?.destroy();}catch{}player=null;frame=null;stage.replaceChildren();label();const a=queue[index];if(!a)return;notify('loading');
